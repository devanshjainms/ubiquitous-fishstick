...
# Plan the terraform
terraform -chdir="$terraform_directory" plan -compact-warnings -json -no-color -parallelism=5 -out=tfplan.tfstate -var="tenant_id=$ARM_TENANT_ID" -var="subscription_id=$ARM_SUBSCRIPTION_ID" -var="client_id=$ARM_CLIENT_ID" -var="client_secret=$ARM_CLIENT_SECRET" -var="object_id=$ARM_OBJECT_ID" -var="location=$Env:LOCATION" -var="environment=$Env:SAP_ENVIRONMENT" -var="virtual_network_id=$Env:VIRTUAL_NETWORK_ID" -var="subnet_address_prefixes=$Env:SUBNET_ADDRESS_PREFIX"

# Apply the terraform
terraform -chdir="$terraform_directory" apply -auto-approve  -compact-warnings -json -no-color -parallelism=5 -var="tenant_id=$ARM_TENANT_ID" -var="subscription_id=$ARM_SUBSCRIPTION_ID" -var="client_id=$ARM_CLIENT_ID" -var="client_secret=$ARM_CLIENT_SECRET" -var="object_id=$ARM_OBJECT_ID" -var="location=$Env:LOCATION" -var="environment=$Env:SAP_ENVIRONMENT" -var="virtual_network_id=$Env:VIRTUAL_NETWORK_ID" -var="subnet_address_prefixes=$Env:SUBNET_ADDRESS_PREFIX"